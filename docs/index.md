...
* 获 FDA 批准的 72 个用于临床肿瘤诊断、指导用药和预后的生物标志物；
* 从 745 篇已发表科研文献中手动收集的 1232 个具有临床应用潜能的生物标志物；
* 从公开发表的 4023 套液体活检 RNA-seq 数据中鉴定出来的约 45000 个候选 RNA 生物标志物。
...
* **I期. 新型标志物的“筛选”阶段**
    * 鲁志实验室负责从上述已构建液体活检生物标志物数据库中，综合考虑标志物的一致性报道个数、已验证的样本数、生物学功能和诊断效果等，筛选出证据等级和临床应用潜力较高的候选生物标志物，涵盖DNA，RNA，蛋白等不同分子类型。
* **II期. 新型标志物的“验证”阶段**
    * 贵单位负责在少量（30-50例）癌症病人和健康对照人群的血浆样本中对上述筛选的标志物进行初步验证；
    * 鲁志实验室和贵单位共同根据初步验证的结果对标志物进行二次筛选；
    * 贵单位负责对二次筛选的标志物在大量（200-500例）癌症病人和健康对照人群的血浆样本中进行更大规模的验证；
    * 鲁志实验室负责对验证数据进行分析，采用机器学习等方法构建标志物诊断模型。
...
`Github: https://github.com/lulab/Ribowave`
    ...
    RiboWave 用于核糖体测序数据（Ribo-seq）的分析。利用小波变换模型来提取核糖体3-nt周期（即信号频率）来使原始信号去噪声化，并精确定位其足迹信息。
...
> `Github: https://github.com/lulab/COME`
    ...
    COME用于mRNA和lncRNA中蛋白编码潜能的分析。

  * RNAfeature

    > Long Hu, et al. Nucleic Acids Research (2015)
`https://github.com/lulab/RNAfeature`

    RNAfeature用于多个物种间非编码RNA保守性区域的分析。
...
> `Github: https://github.com/lulab/RBPgroup`
    ...
    RBPgroup用于各种CLIP-seq数据的RNA结合蛋白的分析。
...
> `Host: http://lulab.life.tsinghua.edu.cn/postar`
    ...
    POSTAR是一个通过RNA结合蛋白来探究转录后调控事件的分析平台。能提供多个物种间RNA-蛋白的相互作用信息。
