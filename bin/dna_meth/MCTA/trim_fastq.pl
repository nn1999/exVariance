...
if ($end == 2) {
    ...
    chomp(my $file_1 = `ls $indir/${sample}_1.fastq.gz`);
    chomp(my $file_2 = `ls $indir/${sample}_2.fastq.gz`);
...
#__________________________________________for the single end sequencing___________________________________________#
    ...
    chomp(my $file = `ls $indir/${sample}_1.fastq.gz`);
    ...
    open OUT_1, ">:gzip", "$outdir/$sample.clean.fastq.gz" or die $!;
